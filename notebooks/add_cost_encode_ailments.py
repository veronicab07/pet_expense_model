...
# -*- coding: utf-8 -*-
"""
Created on Sun Jul  7 00:50:50 2024
...
df3 = pd.read_csv('dogs_expenses_modelling.csv')

# ------------ AILMENTS PRE-PROCESSING ----------------

# Drop 'severity_scale' column
ailments.drop('severity_scale', axis=1, inplace=True)
...
ailments['total_cost'] = ailments['average_cost'] + (ailments['average_cost'] * ailments['recurring_cost'] * 3)

# Split ailments into 5 classes within the new column 'severity_scale'
ailments['severity_scale'], bins = pd.qcut(ailments['total_cost'], 5, labels=[1, 2, 3, 4, 5], retbins=True, duplicates='drop')

# Drop now unnecessary cost columns
ailments.drop(columns=['real_cost_min', 'real_cost_max','average_cost', 'total_cost', 'recurring_cost', 'size_multiplier'], axis=1, inplace=True)
...
def calculate_dynamic_severity(row, ailment_to_severity_scale):
    severity_score = 0
    for ailment, presence in row.items():  # Iterate through each ailment in the row
        if presence and ailment in ailment_to_severity_scale:  # Check if the breed has this ailment and it's a known ailment
            severity_score += ailment_to_severity_scale[ailment]  # Add its severity scale to the score
    return severity_score

# Applying the function to each row in df_breed to calculate the severity_score.
df_breed['severity_score'] = df_breed.apply(calculate_dynamic_severity, axis=1, args=(ailment_to_severity_scale,))
